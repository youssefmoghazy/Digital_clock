...
ERROR Timers_enuTimer1Init(void)
{
	...
	/* Select HW Action on OC1A pin => Non Inverting (clear on compaer) */
...
ERROR Timers_enudelayms(u32 milliseconds)
{
	...
	SET_BIT(TCCR0, CS00);
	#endif
	    u16 OVFTime = 256 * prescaler / F_CPU;
	    u16 numOfOVF = (milliseconds * 1000) /OVFTime;
	    u16 proLoad = (((milliseconds * 1000) % OVFTime) * F_CPU) / 8;
	    TCNT0 = 256 - proLoad;
	    u16 counter = 0;
	    while (counter != (numOfOVF + 1))
	    {
	    ...
	    TCCR0 &= CLEAR_CLK_SOURCE;
}
/******************************************/
/*ERROR Timers_enuTimer1SetTimerValue(u16 Copy_u8Value)
{
	TCNT1 = Copy_u8Value ;
	return STD_OK ;
}
